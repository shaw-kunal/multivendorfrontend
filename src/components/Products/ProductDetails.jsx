                ...
                <div className='flex  flex-col 800px:flex-row gap-3 px-2 py-2'>
                    <div className='flex-1 flex flex-col gap-4 '>
                        <div className='w-full h-[350px] overflow-hidden'>
                            <img className='w-[80%]  object-cover mx-auto' src={import.meta.env.VITE_IMAGE + data?.images[select]} alt="" />
                        </div>
                        <div className='flex gap-2'>
                            <div onClick={() => setSelect(0)} className={`w-full ${select === 0 ? "border" : ""} cursor-pointer border-cyan-400`}> <img  className=' w-full h-full object-cover' src={import.meta.env.VITE_IMAGE + data?.images[0]} alt="not found" /></div>
                            <div onClick={() => setSelect(1)} className={`w-full ${select === 1 ? "border" : ''} cursor-pointer border-cyan-400`}> <img className=' w-full h-full object-cover' src={import.meta.env.VITE_IMAGE  + data?.images[1]} alt="not found" /></div>
                        </div>
...
const ProductDetailsInfo = ({ data }) => {
    ...
    useEffect(() => {
        const getProductCount = async () => {
            try {
                const res = await axios.get(import.meta.env.VITE_PROXY + `/product/count/${data.shopId}`);
                ...
                <div className="w-full bg-slate-300 rounded-b-md flex flex-col 800px:flex-row  gap-5 p-5 space-y-6">
                    <div className="w-full 800px:w-2/3 ">
                        <div className='flex  items-center'>
                            ...
                            <img className="w-10 h-10 rounded-full" src={import.meta.env.VITE_IMAGE + data?.shop?.avatar} alt="" />

                            <div className='m-4 '>
                                <p className='text-blue-400'>{data?.shop?.name}</p>
                                <p className='font-semibold'>({data.shop.ratings || "4/5"} ) Ratings</p>
                            </div>
                        </div>
                        <p>{data.shop.description || "Lorem Ipsum is in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum."}</p>
                    </div>
